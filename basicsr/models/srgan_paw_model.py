...
from losses.loss_util import get_refined_artifact_map, backprojection, min_ce_loss
...
class SRGANPAWModel(SRModel):
    ...
    def optimize_parameters(self, current_iter):
        ...
        self.output, self.preds, self.probs = self.net_g(self.lq)
        if self.cri_ldl:
        ...
        if hasattr(self, 'coeff'):
            ...
            self.pos_weight = get_weight(self.gt, self.output, self.preds, self.probs)
        ...
        if (current_iter % self.net_d_iters == 0 and current_iter > self.net_d_init_iters):
            ...
            if self.cri_pix:
                ...
                # ADD OTHER LOSS AUTOMATICALLY
                preds_down = backprojection(self.preds, self.lq)
                lr_repeat = self.lq.unsqueeze(1).repeat(1, preds_down.size(1), 1, 1, 1)
                l_g_back = self.cri_pix(preds_down, lr_repeat)
                l_g_total += l_g_back
                loss_dict['l_g_back'] = l_g_back

                l_g_sample = min_ce_loss(self.gt, self.preds, self.probs)
                l_g_total += l_g_sample
                loss_dict['l_g_back'] = l_g_sample
