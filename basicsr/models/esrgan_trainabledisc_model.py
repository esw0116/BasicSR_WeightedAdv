...
class ESRGANTrainDModel(SRModel):
    ...
    def init_training_settings(self):
        ...
        if load_path is not None:
            ...
            self.load_network(self.net_wd, load_path, self.opt['path'].get('strict_load_w', True), param_key)
        ...
        if (current_iter % self.net_d_iters == 0 and current_iter > self.net_d_init_iters):
            ...
            if self.cri_pix:
                if self.pos_weight is None:
                    ...
                    l_g_pix = self.cri_pix(self.output, self.gt, pos_weight=1-weight_vgg*self.pos_weight.detach())
                l_g_total += l_g_pix
                loss_dict['l_g_pix'] = l_g_pix
            # perceptual loss
            if self.cri_perceptual:
                l_g_percep, l_g_style = self.cri_perceptual(self.output, self.gt, pos_weight=1-weight_vgg*self.pos_weight.detach())
            ...
            l_g_real = self.cri_gan(real_d_pred - torch.mean(fake_g_pred), False, is_disc=False, pos_weight=self.pos_weight.detach())
            l_g_fake = self.cri_gan(fake_g_pred - torch.mean(real_d_pred), True, is_disc=False, pos_weight=self.pos_weight.detach())
    ...
    def save(self, epoch, current_iter):
        ...
        self.save_network(self.net_wd, 'net_w', current_iter)
